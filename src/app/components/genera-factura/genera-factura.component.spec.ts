...
it('debe retornar la fecha de la factura desde ventaTapete.ticket.fecha cuando existe', () => {
  const fecha = component.getFechaFactura();
...
it('debe generar conceptos y traslados correctamente al llamar llenaFactura()', () => {
  ...
  component.ventaTapete.detalle = [prod];

  // Ejecuta el método
  const timbrado = component.llenaFactura();

  // Verifica que se generó un concepto
  expect(timbrado.Conceptos.length).toBe(1);
  expect(timbrado.Conceptos[0].ClaveProdServ).toBe('10101504');
  expect(timbrado.Conceptos[0].Cantidad).toBeCloseTo(2, 1);
  expect(timbrado.Conceptos[0].Impuestos.Traslados.length).toBe(1);

  // Verifica que el traslado tiene los valores esperados
  const traslado = timbrado.Conceptos[0].Impuestos.Traslados[0];
  expect(traslado.Impuesto).toBe('002');
  expect(traslado.TasaOCuota).toBeCloseTo(0.160000, 5);
  expect(traslado.Importe).toBeCloseTo(32, 1); // 100 * 0.16 * 2 = 32
});
